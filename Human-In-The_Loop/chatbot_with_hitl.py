...
def get_stock_price(symbol: str) -> dict:
    ...
    using DuckDuckGo search results and return a structured response.
    """
    query = f"{symbol} stock price latest"
    try:
        results = search_tool.run(query)
        price_match = re.search(r"\$?\d{1,3}(?:,\d{3})*(?:\.\d+)?", results or "")
